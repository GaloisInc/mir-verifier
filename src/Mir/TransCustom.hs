...
saturateValueUnsigned w Add = Just $ R.App $ E.BVLit w (BV.mkBV w (shift 1 (fromInteger $ C.intValue w) - 1))
saturateValueUnsigned w Sub = Just $ R.App $ E.BVLit w (BV.mkBV w 0)
saturateValueUnsigned w Mul = Just $ R.App $ E.BVLit w (BV.mkBV w (shift 1 (fromInteger $ C.intValue w) - 1))
...
saturateValueSigned w op pos = case op of
  ...
  where
    ...
    maxVal = R.App $ E.BVLit w (BV.mkBV w ((1 `shift` (bits - 1)) - 1))
    minVal = R.App $ E.BVLit w (BV.mkBV w (negate $ 1 `shift` (bits - 1)))
...
makeSaturatingArith name bop =
  ...
  where
    ...
    isPos (MirExp (C.BVRepr w) e) = return $ R.App $ E.BVSle w (R.App $ E.BVLit w (BV.mkBV w 0)) e
  ...
  where
    ...
    buildMux w i w' bv = case isZeroNat i of
        ZeroNat ->
            ...
            -- zeros.
            E.BVLit w' $ BV.mkBV w' (intValue w)
        NonZeroNat
          | i' <- predNat i
          ...
          -> E.BVIte (R.App $ getBit w i' bv) w'
                (R.App $ E.BVLit w' $ BV.mkBV w' (intValue w - intValue i))
...
type_id = (["core","intrinsics", "", "type_id"],
  \ _substs -> Just $ CustomOp $ \ opTys ops ->
    ...
    return $ MirExp knownRepr $ R.App (E.BVLit (knownRepr :: NatRepr 64) (BV.mkBV knownNat 0)))
...
bv_funcs =
    ...
    , bv_eq
    , bv_lt
    , bv_literal "ZERO" (\w -> E.BVLit w (BV.mkBV w 0))
    , bv_literal "ONE" (\w -> E.BVLit w (BV.mkBV w 1))
    , bv_literal "MAX" (\w -> E.BVLit w $ BV.mkBV w ((1 `shift` fromIntegral (intValue w)) - 1))
    , bv_leading_zeros
    ]
